...
class Security {
  ...
  checkRequestMethod(req, res, ip) {
    if (!this.config.security.allowedMethods.includes(req.method)) {
      ...
      this.logger.log(
        'warning',
        `Invalid method ${req.method} from ${ip}`,
        req
      );
      return false;
    }
  ...
  isBlacklisted(req, res, ip) {
    ...
    if (tempBlock && tempBlock.expiry > Date.now()) {
      this.logger.log(
        'blocked',
        `Blocked request from temporarily blocked IP ${ip}`,
        req
      );
      return true;
    }
      ...
      for (const [category, patterns] of Object.entries(this.advancedPatterns)) {
        for (const pattern of patterns) {
          try {
            ...
            this.logger.log(
              'warning',
              `Advanced pattern test error in ${category}: ${e.message}`,
              req
            );
          }
        }
  ...
  calculateBlockDuration(ip, score) {
    ...
    let duration = Math.min(
      30 *
        60 *
        1000 *
        Math.pow(this.thresholds.blockIncrementFactor, blockCount),
  ...
  incrementAttackScore(ip, score) {
    ...
    if (now - lastTimestamp < 60000) {
      ...
      if (currentScore + score >= this.thresholds.maxAttemptsPerMinute) {
        ...
        this.logger.log(
          'banned',
          `IP ${ip} permanently banned due to high attack score`
        );
      }
    }
